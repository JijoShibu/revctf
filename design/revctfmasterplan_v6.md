...
| D13 | **`--sandbox` is ON by default for the executing stages; `--no-sandbox` opts out. With no Docker, those stages are SKIPPED, never run on the host** | v5 §3 and v6 §9, where `--sandbox` is an opt-in flag and the executing stages otherwise run directly on the host | The default was backwards for a tool aimed at beginners. A beginner's first action is to download a challenge binary and scan it, and revctf ran `ltrace` and `strace` on it — executing it on the host — unless the user knew to pass a flag whose existence they had no reason to suspect. Meanwhile `--sandbox` *refused* those stages rather than isolating them (M6 was unbuilt), so the safe option did not work and the unsafe option was the default. picoCTF binaries are benign; a binary from an arbitrary CTF, a shared drive or a writeup repo is not, and the user cannot know before running it. **The obvious objection is D7:** making Docker mandatory for a default scan would turn a missing tool into a hard startup error and stop scanning entirely on hosts without it. The third path avoids that — Docker is *not* a hard dependency, because a scan without it still completes; it simply omits `ltrace`/`strace` and says so, exactly as Tier C degrades Ghidra to radare2 and FLOSS to `--only static`. The governing principle: **it degrades to not executing, never to executing unsafely.** **Explicitly rejected: silently falling back to the host when Docker is absent.** That would make the same command a security boundary on one machine and not on another while the user believes they are isolated in both cases, and a false sense of isolation is worse than a known absence of one. The skip reason must therefore name Docker as the cause and `--no-sandbox` as the deliberate override — otherwise it reads as "these stages did not apply to this file", which is a different statement entirely. `--no-sandbox` runs them on the host and the report states plainly that the target was executed without isolation. CLI surface 27 → 28 flags. |
