...
### The rest of that run, folded in from the raw capture

`host-measurements-jijo-20260820.txt` sat in the repo root carrying these numbers plus a
hostname and a machine spec no user has any use for. The numbers are here now and the file
is deleted; `tools/measure-host.sh` regenerates it on demand for anyone who needs a fresh
one.

Wall-clock and peak RSS, same host and date, `[GNU time]`:

| run | peak | wall |
|---|---|---|
| `crackme`, all stages | 345MB | 9.7s |
| `crackme`, `--skip-ghidra` | 101MB | 3.1s |
| `large_blob.bin` (220MB), `--skip-ghidra` | 104MB | 35.4s |

Two things worth keeping from those. **Ghidra is the entire memory story on a small
target** — 345MB with it, 101MB without, on the same 16KB binary. And on the 220MB blob
...
### What the sandbox actually changes, and the two things that nearly went wrong

`--sandbox` had existed as a flag since M1 and did nothing but refuse. Making it work turned
up two problems that are invisible from the outside, because in both cases the scan
succeeds, the traces arrive and the report is correct — only the guarantee is missing.

**1. `systemd-run --scope` does not reach into a container.** `st_run_bounded` wraps what it
launches in the tier's memory scope. Under the sandbox, what it launches is the *docker
client*: the container is forked by `dockerd` and lives in a different cgroup entirely. So
the scope would have bounded a client that uses a few MB and bounded the traced target at
nothing — the exact "reported but not enforced" shape M5 existed to eliminate, one level
further out and completely invisible in the report. The ceiling from
`tier_ceiling_for_stage` is therefore passed to `docker run --memory`, with `--memory-swap`
pinned to the same figure so the container cannot buy the headroom back through swap. The
m6 section asserts `--memory <n>m` appears in the capture.

**2. The orphan sweep does not apply, and `timeout` alone leaks a live container.**
`dyn_sweep_orphans` signals a process *group*; under the sandbox `ST_LAST_PGID` is the
docker client's group and the container is not in it. Worse, `timeout` firing kills the
client while the container keeps running the hostile target with nothing watching it —
`--rm` covers only a clean exit. Teardown is therefore `docker rm -f <name>`, by a
deterministic per-run name, run unconditionally after the tracer returns.

### The contract is printed, not summarised

`stage_record_exec` records the command line, but `lib/report.sh` prints `STAGE_CMD` only
in DIAGNOSTICS — i.e. only for a stage that FAILED. So on every successful sandboxed run the
banner's claim ("no network, read-only filesystem, all capabilities dropped") would have had
no evidence under it, and the harness would have had nothing to grep but revctf's own
adjectives. `dyn_banner` now prints the real flag list, generated by the same `sbx_wrap` the
run used. This was chosen over adding a report section: the scope rule says a fix must not
cost more than the defect, and one printf beats a new section.

### The egress test needs a positive control or it is worthless

"Assert the target cannot reach the network" passes trivially on any host with no outbound
connectivity, which is the same vacuous shape that has bitten this project four times. The
check therefore runs the *identical* probe (`exec 3<>/dev/tcp/1.1.1.1/53`, a raw IP so no
DNS is involved) twice: once with `--network=bridge`, where it must SUCCEED, and once under
the real contract, where it must FAIL. If the bridge run also fails, the host cannot
demonstrate egress and the check SKIPs with that reason. It can never pass by default.

The `sandbox_bypass` mutation deletes `--network=none` from `sbx_wrap` and asserts three
checks flip. Two of them look interchangeable and are not: "the isolation contract is
printed" proves revctf *types* the flag, "no network egress" proves the flag *does*
something. Only the second is a security property.

### pyinstxtractor is fetched, not vendored — and why

M6's DoD said *vendor* it. It is GPLv3, and revctf declares no licence of its own.
Committing GPLv3 source into an unlicensed repository decides revctf's licensing as a side
effect of a convenience, which is not a call `install.sh` should make quietly. revctf runs
it as a separate process, so nothing is linking against it. `install.sh` fetches it into
`scripts/` during its network window and validates the body (a captive portal or a 404 page
arrives as a perfectly successful 200, and an HTML file named `.py` fails later with a
SyntaxError that points at revctf rather than at the download). If revctf later adopts a
compatible licence, vendoring the file is strictly better — it would work offline — and this
step goes away.

### A stale DOCKER_HOST looks exactly like a dead daemon

Cost real time on the development VM: `DOCKER_HOST` pointed at a podman socket that no
longer existed, so every docker call failed while `systemctl status docker` said active.
`sbx_available` names the variable in its failure sentence, and `install.sh` names the
`docker` group (membership is per-process, so it does not take effect in the shell that ran
