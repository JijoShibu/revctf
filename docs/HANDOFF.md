...
`device_bash` has no network. Delivery was therefore **git bundle → user pushes**.

**Superseded as of 2026-08-26.** Development moved to the Kali VM, `gh` 2.98.0 is
installed at `~/.local/bin/gh`, and pushes and releases go direct to GitHub. The
PUSH-RUNBOOK.md this paragraph used to reference was never committed to the repo; the
bundle workflow it described is no longer needed. The constraint above is kept only as a
record of why the early milestones were delivered the way they were.
