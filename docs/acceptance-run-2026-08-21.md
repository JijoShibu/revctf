...
`acceptance-artifacts/` as the "before" half of the M6 sandbox before/after pair. **Those
capture directories were removed from the repo before the public release** — 5.2MB of raw
tool output carrying absolute `/home/<user>` paths, whose conclusions are all recorded in
this document. Re-run the scans to regenerate them.
