...
**Pass condition** — match `acceptance-artifacts/sandboxed/unpackme/report.txt` exactly:
...
diff ~/rehearsal/06-flags.txt \
     <(sed -n '/POSSIBLE FLAGS/,/^=\{10,\}/p' ~/revctf/acceptance-artifacts/sandboxed/unpackme/report.txt)
```
...
Do not compare against `acceptance-artifacts/unpackme/` (no `after/`, no `sandboxed/`) —
that is the *before* run from 2026-08-21, recorded when the stack-string decoder did not
exist yet and the flag genuinely was not found. It is kept as history, and its `0 high` is
the correct result for the code of that date.
