...
#
# 64MB matches the radare2 deep-analysis limit and keeps FLOSS comfortably inside every
# tier. A target that large is firmware or a data blob, where FLOSS's stack-string
# emulation has nothing meaningful to find anyway.
...
stage_floss() {
    ...
    if [[ $size_mb -gt $FLOSS_MAX_MB ]]; then
        {
            ...
            printf 'which would breach the memory ceiling on every hardware tier.\n'
    ...
    local -a modes; local scope_note
    ...
    if [[ $rc -eq 124 || $rc -eq 137 ]]; then
        stage_set_status "$name" failed "timed out after ${ST_T_FLOSS}s (partial output kept)"
    elif [[ $rc -ne 0 ]]; then
